...
export default function ScheduledCheckIns({ onEmpty }: Props) {
  ...
  async function dismissStep(flowId: string, stepNumber: number) {
    const key = `${flowId}-${stepNumber}`
    if (dismissing.has(key)) return
    setDismissing((prev) => new Set(prev).add(key))
    // Optimistic remove
    setDueFlowSteps((prev) => prev.filter((s) => !(s.flow_id === flowId && s.step_number === stepNumber)))
    try {
      const res = await fetch(`/api/client/autoflows/${flowId}/${stepNumber}/dismiss`, { method: 'POST' })
      if (!res.ok) throw new Error('dismiss failed')
    } catch {
      // Revert on failure: reload the dashboard list
      const flowRes = await fetch('/api/client/autoflows/due')
      if (flowRes.ok) {
  ...
  return (
    <section>
      ...
      {dueFlowSteps.filter(s => s.show_as_checkin_prompt).map((step) => (
        <div key={`${step.flow_id}-${step.step_number}`} className="relative mb-3">
          <a
            href={`/autoflows/${step.flow_id}/${step.step_number}`}
            className="flex items-center justify-between rounded-2xl px-5 py-4 hover:opacity-90 transition-colors group" style={{ backgroundColor: 'rgba(29,158,117,0.07)', border: '1px solid rgba(29,158,117,0.18)' }}
          >
            ...
            <span className="text-xs font-semibold text-white px-3 py-1.5 rounded-lg flex-shrink-0 mr-7" style={{ backgroundColor: '#1D9E75' }}>Fill in →</span>
          </a>
          <button
            type="button"
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); dismissStep(step.flow_id, step.step_number) }}
            disabled={dismissing.has(`${step.flow_id}-${step.step_number}`)}
            aria-label="Dismiss for 1 week"
            title="Dismiss for 1 week"
            className="absolute top-2 right-2 w-6 h-6 rounded-full flex items-center justify-center text-gray-400 hover:text-gray-700 hover:bg-white/60 transition-colors disabled:opacity-40"
          >
